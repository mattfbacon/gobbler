...
using ArgumentMismatchReason = ArgumentMismatchError::Reason;
...
static ArgType identify_arg_type(std::string_view const arg) {
	if (!arg.starts_with("-")) {
...
static bool check_next_argument(arg_iter_t& arg_iter, arg_iter_t const& arg_end_iter) {
...
static void parse_short_option_block(Callbacks& callbacks, arg_t short_option_block, arg_iter_t& arg_iter, arg_iter_t const& arg_end_iter) {
	do {
			...
			case OptionCallbackResult::want_argument: {
				...
				if (option_argument.empty()) {  // `-b org` instead of `-borg`
					if (check_next_argument(arg_iter, arg_end_iter)) {
						...
						throw ArgumentMismatchError{ ArgumentMismatchReason::expected_not_provided, short_option_name };
					}
				}
...
static void parse_long_option(Callbacks& callbacks, arg_t long_option_content, arg_iter_t& arg_iter, arg_iter_t const& arg_end_iter) {
	...
	switch (option_callback_result) {
		case OptionCallbackResult::want_argument: {
			...
			if (equals_sign_position != decltype(long_option_content)::npos) {
				...
				if (check_next_argument(arg_iter, arg_end_iter)) {
					...
					throw ArgumentMismatchError{ ArgumentMismatchReason::expected_not_provided, long_option_name };
				}
			}
		...
		case OptionCallbackResult::no_argument:
			if (equals_sign_position != decltype(long_option_content)::npos) {  // argument provided to option that takes no arguments
				throw ArgumentMismatchError{ ArgumentMismatchReason::provided_not_expected, long_option_name };
			}
			break;
	}
}
